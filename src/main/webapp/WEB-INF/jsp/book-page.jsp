...
<head>
    ...
    <script src="<c:url value="/resource/js/comment.js"/>"></script>
    ...
    <div class="row">
            ...
            <div class="profile-img">
                <c:choose>
                    <c:when test="${requestScope.book.cover == null || requestScope.book.cover eq ''}">
                        <img style="height: 225px; width: 225px;" class="card-img-top"
                             src="<c:url value="/resource/img/default-book-cover.jpg"/>"
        ...
        <div class="col-md-6">
            <div class="profile-head">
                ...
                <p class="proile-rating text-white">Publisher: <span
                        class="text-white">${requestScope.book.publisher}</span></p>
    ...
    <div class="row">
        <div class="col-md-4">
            <div class="profile-work">
                <br>
                    <c:if test="${sessionScope.user.role.roleId == 2 && requestScope.book.availableAmount!=0}">
                            <button onclick="document.getElementById('calendar').style.display='block'"
                                    class="btn btn-success" style="width: 100%;">Borrow book
                            </button>

                        <div id="calendar" id="modal-1" class="modal">

                            <form action="controller" id="modal-content-1" class="modal-content animate bg-dark"
                                  method="post">
                                ...
                                <div id="cont-calendar1" class="container bg-dark">
                                    <input type="date" name="dueDate" required>
                                </div>

                                <div id="cont-calendar" class="container bg-dark">

                                        <button type="submit" id="borrow-button" class="btn btn-success" name="command"
                                                value="add-borrow-record">Submit
                                        </button>

                                    <button type="button"
                                            onclick="document.getElementById('calendar').style.display='none'"
                                            class="btn btn-danger cancelbtn" >Cancel
                                    </button>
                                </div>
                            </form>
                        </div>
                    ...
                    <c:if test="${sessionScope.user.role.roleId == 1}">
                        <form action="controller" method="post">
                            <input type="hidden" name="bookId" value="${requestScope.book.id}">
                            <button style="width: 100%;" type="submit" name="command" value="change-book-deleted-status"
                        ...
                        <form action="controller" method="get">
                            <input type="hidden" name="bookId" value="${requestScope.book.id}">
                            <button style="width: 100%;" type="submit" name="command" value="forward-edit-book"
            ...
            <div class="tab-content profile-tab" id="myTabContent">
                ...
                <div class="tab-pane fade" id="profile" role="tabpanel" aria-labelledby="profile-tab">
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <c:choose>
                                <c:when test="${requestScope.book.availableAmount == 0}">
                                    <p class="text-white">Unavailable</p>
                                </c:when>
                                <c:otherwise>
                                    <p class="text-white">Available (${requestScope.book.availableAmount} out
                                        of ${requestScope.book.totalAmount})</p>
                                </c:otherwise>
                            </c:choose>
                        </div>
                    </div>
